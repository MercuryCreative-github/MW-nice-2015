...
function timetable_events_init()
{
	global $wpdb;
	$labels = array(
		'name' => _x('Forums', 'post type general name', 'timetable'),
		'singular_name' => _x('Forum', 'post type singular name', 'timetable'),
		'add_new' => _x('Add New', 'Forum', 'timetable'),
		'add_new_item' => __('Add Forum', 'timetable'),
		'edit_item' => __('Edit Forum', 'timetable'),
		'new_item' => __('New Forum', 'timetable'),
		'all_items' => __('All Forums', 'timetable'),
		'view_item' => __('View Forums', 'timetable'),
		'search_items' => __('Search Forum', 'timetable'),
		'not_found' =>  __('No Forum found', 'timetable'),
		'not_found_in_trash' => __('No Forums found in Trash', 'timetable'),
		'parent_item_colon' => '',
		'menu_name' => __("Agenda Forums", 'timetable')
	);
	$args = array(
		"labels" => $labels,
		"public" => true,
...
function timetable_inner_events_custom_box_side($post)
{
		...
		for($i=0; $i<$event_hours_count; $i++)
		{
			...
			if($event_hours[$i]->tooltip!="" || $event_hours[$i]->before_hour_text!="" || $event_hours[$i]->after_hour_text!="" || $event_hours[$i]->category!="")
			{
				...
				if($event_hours[$i]->tooltip!="")
					//echo '<br /><strong>' . __('Presentations', 'timetable') . ':</strong> ' . $event_hours[$i]->tooltip;
				if($event_hours[$i]->before_hour_text!="")
					echo '<br /><strong>' . __('Title', 'timetable') . ':</strong> ' . $event_hours[$i]->before_hour_text;
				if($event_hours[$i]->after_hour_text!="")
					echo '<br /><strong>' . __('Chair', 'timetable') . ':</strong> ' . $event_hours[$i]->after_hour_text;
				if($event_hours[$i]->category!="")
					echo '<br /><strong>' . __('Day', 'timetable') . ':</strong> ' . $event_hours[$i]->category;
				echo '</div>';
			}
			echo '</li>';
		}
	...
	echo '		</select>
		...
		<tr>
			<td>
				<label for="before_hour_text">' . __('Title', 'timetable') . ':</label>
			</td>
			<td>
	...
	<tr>
			<td>
				<label for="after_hour_text">' . __('Chair', 'timetable') . ':</label>
			</td>
			<td>
		...
		<tr>
			<!--td>
				<label for="tooltip">' . __('Presentations', 'timetable') . ':</label>
			</td>
			<td>
		...
		<tr>
			<td>
				<label for="event_hour_category">' . __('Day', 'timetable') . ':</label>
			</td>
			<td>
