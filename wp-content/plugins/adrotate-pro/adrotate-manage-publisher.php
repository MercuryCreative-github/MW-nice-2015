...
function adrotate_insert_input() {
	...
	if(wp_verify_nonce($_POST['adrotate_nonce'], 'adrotate_save_ad')) {
		...
		if(current_user_can('adrotate_ad_manage')) {
			...
			$wpdb->update($wpdb->prefix.'adrotate', array('title' => $title, 'bannercode' => $bannercode, 'updated' => $thetime, 'author' => $author, 'imagetype' => $imagetype, 'image' => $image, 'link' => $link, 'tracker' => $tracker, 'responsive' => $responsive, 'weight' => $weight, 'sortorder' => $sortorder, 'cbudget' => $cbudget, 'ibudget' => $ibudget, 'crate' => $crate, 'irate' => $irate, 'cities' => $cities, 'countries' => $countries), array('id' => $id));
...
function adrotate_insert_schedule() {
	...
	if(wp_verify_nonce($_POST['adrotate_nonce'], 'adrotate_save_schedule')) {
		...
		if(current_user_can('adrotate_schedule_manage')) {	
			...
			$linkmeta = $wpdb->get_results($wpdb->prepare("SELECT `ad` FROM `".$wpdb->prefix."adrotate_linkmeta` WHERE `schedule` = %d AND `block` = 0 AND `user` = 0;", $id));
			foreach($linkmeta as $meta) {
				$meta_array[] = $meta->ad;
			}
			...
			$insert = array_diff($ads, $meta_array);
			foreach($insert as &$value) {
				$wpdb->insert($wpdb->prefix.'adrotate_linkmeta', array('ad' => $value, 'group' => 0, 'block' => 0, 'user' => 0, 'schedule' => $id));
			}
			unset($value);
			...
			$delete = array_diff($meta_array, $ads);
			foreach($delete as &$value) {
				$wpdb->query($wpdb->prepare("DELETE FROM `".$wpdb->prefix."adrotate_linkmeta` WHERE `ad` = %d AND `group` = 0 AND `block` = 0 AND `user` = 0 AND `schedule` = %d;", $value, $id)); 
			}
			unset($value);
	...
	if(wp_verify_nonce($_POST['adrotate_nonce'], 'adrotate_save_media')) {
		if(current_user_can('adrotate_ad_manage')) {
			if($_FILES["adrotate_image"]["size"] > 0 AND $_FILES["adrotate_image"]["size"] <= 512000) {
				...
				$extension = end(explode(".", $_FILES["adrotate_image"]["name"]));
