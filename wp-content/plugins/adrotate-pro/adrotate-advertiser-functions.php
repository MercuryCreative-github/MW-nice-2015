...
function adrotate_get_template_part($slug, $name = '') {
	$path = WP_CONTENT_DIR.'/plugins/'.ADROTATE_FOLDER.'/templates/';
...
function adrotate_load_adverts($user_id) {
	...
	$ads = $wpdb->get_results($wpdb->prepare("SELECT `ad` FROM `".$wpdb->prefix."adrotate_linkmeta` WHERE `group` = 0 AND `block` = 0 AND `user` = %d ORDER BY `ad` ASC;", $user_id));
