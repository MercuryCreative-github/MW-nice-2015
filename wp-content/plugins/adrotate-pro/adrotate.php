	...
	<div class="wrap">
		...
		<?php if($wpdb->get_var("SHOW TABLES LIKE '".$wpdb->prefix."adrotate';") AND $wpdb->get_var("SHOW TABLES LIKE '".$wpdb->prefix."adrotate_groups';") AND $wpdb->get_var("SHOW TABLES LIKE '".$wpdb->prefix."adrotate_schedule';") AND $wpdb->get_var("SHOW TABLES LIKE '".$wpdb->prefix."adrotate_linkmeta';")) { ?>
			...
			foreach($allbanners as $singlebanner) {
				...
				if($adrotate_config['enable_advertisers'] == 'Y') {
					$advertiser = $wpdb->get_var("SELECT `user_login` FROM `".$wpdb->prefix."adrotate_linkmeta`, `$wpdb->users` WHERE `$wpdb->users`.`id` = `".$wpdb->prefix."adrotate_linkmeta`.`user` AND `ad` = '".$singlebanner->id."' AND `group` = '0' AND `block` = '0' AND `schedule` = '0' LIMIT 1;");
				}
	  	...
	  	<form name="settings" id="post" method="post" action="admin.php?page=adrotate-settings">
			...
			<table class="form-table">
				<tr>
					...
					<td>
						<label for="adrotate_banner_folder"><?php echo home_url(); ?>/<input name="adrotate_banner_folder" type="text" class="search-input" size="30" value="<?php echo $adrotate_config['banner_folder']; ?>" autocomplete="off" /> <?php _e('(Default: wp-content/banners/).', 'adrotate'); ?><br />
